#!/bin/bash
# linux_setup.sh - Linux Setup Script (Homebrew + Apt)
set -e
...
# --- 6. CLI Tools (Homebrew) ---
echo "Installing CLI Tools via Homebrew..."
CLI_TOOLS=(gh jq fzf ripgrep podman podman-compose)
for tool in "${CLI_TOOLS[@]}"; do install_formula "$tool"; done

gh extension install github/gh-copilot --force || true

# --- 7. Applications (Apt/Snap/Flatpak) ---
echo "Installing GUI Applications..."
sudo snap install code --classic || echo "VS Code already installed via snap."
sudo snap install bruno || echo "Bruno already installed via snap."
sudo apt install -y steam strawberry || true
